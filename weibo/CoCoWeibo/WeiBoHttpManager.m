...
- (NSURL*)generateURL:(NSString*)baseURL params:(NSDictionary*)params {
	if (params) {
        ...
        NSMutableString *su = [NSMutableString stringWithString:url];
        [su appendString:@"&with_offical_account=1"];
		return [NSURL URLWithString:su];
	} else {
...
//发布文字图片微博 腾讯
-(void) postTencentWithText:text image:image
{
    //https://open.t.qq.com/api/t/add_pic 
    NSURL *url = [NSURL URLWithString:@"https://open.t.qq.com/api/t/add_pic"];
    ASIFormDataRequest *item  = [[ASIFormDataRequest alloc]initWithURL:url];
    OAuthManager *oauthManager = [[OAuthManager alloc]initWithOAuthManager:TENCENT_WEIBO];
    NSDictionary *params = [oauthManager getCommonParams]; 
    ...
    [item setPostValue:access_token forKey:@"access_token"];
    [item setPostValue:oauth_consumer_key forKey:@"oauth_consumer_key"];
    [item setPostValue:openid forKey:@"openid"];
    [item setPostValue:clientip forKey:@"clientip"];
    [item setPostValue:oauth_version  forKey:@"oauth_version"];
    [item setPostValue:scope forKey:@"scope"];
    [item addData:UIImagePNGRepresentation(image) forKey:@"pic"];
    [item setPostValue:text         forKey:@"content"];
    [self setPostUserInfo:item withRequestType:TencentPostTextAndImage];
    [requestQueue addOperation:item];
    [item release];
}
...
-(void)postTencentWithText:(NSString*)text
{
    ...
    [item setPostValue:scope forKey:@"scope"];
    [item setPostValue:text         forKey:@"content"];
    [self setPostUserInfo:item withRequestType:TencentPostText];
    [requestQueue addOperation:item];
    [item release];
}
...
-(void)getHomeLine:(int64_t)sinceID maxID:(int64_t)maxID count:(int)count page:(int)page baseApp:(int)baseApp feature:(int)feature
{
    ...
    [dict release];
    [requestQueue addOperation:request];
//  requestQueue setMaxConcurrentOperationCount:2;      //  同时执行的下载线程数
    [request release];
}
...
//    http://open.t.qq.com/api/statuses/home_timeline?format=xml&pageflag=0&pagetime=0&reqnum=20&type=0&contenttype=0x80&oauth_consumer_key=xx&access_token=xx&openid=xx&clientip=xx&oauth_version=2.a&scope=xx
    
    
    [params setObject:format forKey:@"format"];
    [params setObject:@"0" forKey:@"pageflag"];
    [params setObject:@"0" forKey:@"pagetime"];
    [params setObject:@"10" forKey:@"reqnum"];
    [params setObject:@"0" forKey:@"contenttype"];
    [params setObject:oauth_consumer_key forKey:@"oauth_consumer_key"];
...
//      @"https://open.t.qq.com/api"
    NSString * baseUrl = [NSString stringWithFormat:@"%@/statuses/home_timeline",TENCENT_V2_DOMAIN];
    NSURL * url = [self generateURL:baseUrl params:params];
    ASIHTTPRequest * request = [[ASIHTTPRequest alloc]initWithURL:url];
    NSLog(@"Tencent request url:%@",url);
    NSMutableDictionary * dict = [[NSMutableDictionary alloc]init];
    [dict setObject:[NSNumber numberWithInt:TencentGetHomeLine] forKey:USER_INFO_KEY_TYPE];
    [request setUserInfo:dict];
    [dict release];
    [requestQueue addOperation:request];
    [request release];
...
- (void)requestFinished:(ASIHTTPRequest *)request{
    ...
    if (requestType == SinaGetFollowingUserList) {        
        ...
        for (id item in arr) {
            User *user = [[User alloc]initWithJsonDictionary:item];
            NSLog(@"SinaGetFollowingUserList:%@",user.avatarImage);
            [userArr addObject:user];
            [user release];
        }
...
//    }
    ...
    if (requestType == TencentPostTextAndImage || requestType == TencentPostText) {
        Status* sts = [Status statusWithJsonDictionary:userInfo];
        if ([delegate respondsToSelector:@selector(didGetTencentPostResult:)]) {
            [delegate didGetTencentPostResult:sts];
        }
    }
...
//            s.isRefresh = @"YES";
    ...
    //Tencent 获取当前登录用户及其所关注用户的最新微博

    if (requestType == TencentGetHomeLine) {
       NSDictionary * dic = [userInfo objectForKey:@"data"];
        if (dic == nil||[dic isEqual:[NSNull null]]) {
            return;
        }
        NSArray * infoArr = [dic objectForKey:@"info"];
...
//        NSMutableArray * statusArr = [[NSMutableArray alloc]initWithCapacity:0];
        ...
        for (id item in infoArr) {
            ...
            [statuesArr addObject:status];
        }
        NSLog(@"[infoDic objectForKey:%@",statuesArr);
        if ([delegate respondsToSelector:@selector(didGetHomeLine:)]) {
            [delegate didGetHomeLine:statuesArr];
        }
        [statuesArr release];
