...
- (Comment*)initWithJsonDictionary:(NSDictionary*)dic {
	...
	if (self = [super init]) {
		commentId = [dic longLongValueForKey:@"id" defaultValue:-1];
		commentKey = [[NSNumber alloc]initWithLongLong:commentId];
		createdAt = [dic timeValueForKey:@"created_at" defaultValue:0];
		text = [[dic stringValueForKey:@"text" defaultValue:@""] retain];
		
		// parse source parameter
		NSString *src = [dic stringValueForKey:@"source" defaultValue:@""];
		...
		favorited = [dic boolValueForKey:@"favorited" defaultValue:NO];
		truncated = [dic boolValueForKey:@"truncated" defaultValue:NO];
